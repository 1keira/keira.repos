...
RealativeDirection PsdLocation::calcRelativeDirection(Data2Location data2Location)
{
...
RelativePosition PsdLocation::calcRelativePosition(Data2Location data2Location)
{
    ...
    hvPos.lon = PsdMessageDecoder::getInstance()->getSelfSegment().hvCoordinate.longitude;
    hvPos.elevation = 0.0;
...
RoadSegmentClass PsdLocation::calcRoadSegmentClass()
{
    ...
    RoadSegmentClass RoadClass = RoadClass_Unkown;
...
bool PsdLocation::RoMatchSegment(Position3D RoPoint)
{
        ...
        if ((*it)->segIsInTree == true)
        {
            ...
            else
            {
                ...
                for (auto place = vAllCoord.begin(); place != (vAllCoord.end()-1); place++)
                {
                    ...
                    Coord_1.lon = (*place).longitude;
                    Coord_1.elevation = 0.0;

                    Position3D Coord_2;
                    Coord_2.lat = (*(place+1)).latitude;
                    Coord_2.lon = (*(place+1)).longitude;
...
Data2EventList PsdLocation::getPsdRoLocation(Data2Location data2Location)
{
    ...
    printf("[%s] [%d]: RoPos.lat = %lf, RoPos.lon = %lf\n", __FUNCTION__, __LINE__, data2Location.RoPos.lat, data2Location.RoPos.lon);
