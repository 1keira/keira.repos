...
Point GeometricCalc::coordinateTransform(const Position3D refPos, const double heading, const Position3D tarPos)
{
    ...
    double x0 = (RE0 + refPos.elevation) * cos(refPos.lat * M_PI / 180) * cos(refPos.lon * M_PI / 180);
    double y0 = (RE0 + refPos.elevation) * cos(refPos.lat * M_PI / 180) * sin(refPos.lon * M_PI / 180);
    ...
    double x1 = (RE + tarPos.elevation) * cos(tarPos.lat * M_PI / 180) * cos(tarPos.lon * M_PI / 180);
    double y1 = (RE + tarPos.elevation) * cos(tarPos.lat * M_PI / 180) * sin(tarPos.lon * M_PI / 180);
    ...
    double dn = -sin(tarPos.lat * M_PI / 180) * cos(tarPos.lon * M_PI / 180) * dx - sin(tarPos.lat * M_PI / 180) * sin(tarPos.lon * M_PI / 180) * dy + cos(tarPos.lat * M_PI / 180) * dz; //standard X
    double de = -sin(tarPos.lon * M_PI / 180) * dx + cos(tarPos.lon * M_PI / 180) * dy;                                                                                                                                                                                           //standard Y
    double dd = -cos(tarPos.lat * M_PI / 180) * cos(tarPos.lon * M_PI / 180) * dx - cos(tarPos.lat * M_PI / 180) * sin(tarPos.lon * M_PI / 180) * dy - sin(tarPos.lat * M_PI / 180) * dz;
