...
TEST_F(PsdMapTest, deleteSubTreeWithDepthGreaterThan1)
{
    ...
    MapData_3.segIsInTree = false;
    PsdMessageDecoder::getInstance()->setPsdMapData(MapData_3);
    PsdMessageDecoder::getInstance()->segmentManager(true);
    ...
    MapData_4.segIsInTree = false;
    PsdMessageDecoder::getInstance()->setPsdMapData(MapData_4);
    PsdMessageDecoder::getInstance()->segmentManager(true);
    ...
    MapData_5.segIsInTree = false;
    PsdMessageDecoder::getInstance()->setPsdMapData(MapData_5);
    PsdMessageDecoder::getInstance()->segmentManager(true);
    ...
    MapData_6.segIsInTree = false;
    PsdMessageDecoder::getInstance()->setPsdMapData(MapData_6);
    PsdMessageDecoder::getInstance()->segmentManager(true);
    ...
    MapData_7.segIsInTree = false;
    PsdMessageDecoder::getInstance()->setPsdMapData(MapData_7);
    PsdMessageDecoder::getInstance()->segmentManager(true);
    ...
    PsdMap::getInstance()->setmLastSegmentId(4);
    ...
    if ( PsdMessageDecoder::getInstance()->getSelfSegment().curSegmentId !=  PsdMap::getInstance()->getmLastSegmentId())
    {
...
TEST_F(PsdMapTest, mapCreateAndmapUpdateAndmapClear)
{
    ...
    MapData_8.segIsInTree = false;
    PsdMessageDecoder::getInstance()->setPsdMapData(MapData_8);
    PsdMessageDecoder::getInstance()->segmentManager(true);
...
TEST_F(PsdMapTest, curIdIsNotInList)
{
    ...
    struct PsdMapData *ptr = PsdMap::getInstance()->IsCurIdInList();
...
protected:
    ...
    PsdLocation *getInstance()
    {
        if (NULL == pInstance)
        {
            pInstance = PsdLocation::testPsdLocation();
        }
        return pInstance;
    }
    ...
    void TerDown()
    {
        PsdLocation::testPsdLocation(pInstance);
    }
};
...
TEST_F(PsdLocationTest, RelativePositionAndRelativeDirection)
{
    ...
    PsdMessageDecoder::getInstance()->setSelfSegment(curSegment);
    PsdMap::getInstance()->mapCreate();
    // EXPECT_EQ(PsdLocation::getInstance()->testcalcRelativePosition(data2Location), Pos_Behind);
...
/*test calcRoDis2Intersection_1*/
TEST_F(PsdLocationTest, calcRoDis2Intersection_1)
{
    ...
    tSelfSegment curSegment;
    curSegment.curSegmentId = 3;
    curSegment.curRemainLength = 185; //drivedLength = 69
    curSegment.posIsUnique = true;
    curSegment.posLengthErr = Pos_2mTo5m;
    curSegment.hvHeading = 179.33;
    curSegment.hvCoordinate = {40.358150, 116.815109};
    ...
    //case: matched HV segment
    Data2Location data2Location;
    data2Location.headingDelta = 180.0;                                                     //RelativeDir == 
    data2Location.RoPos = {40.357956, 116.815113, 0.0};                     //matched HV segment
    data2Location.RvSpeed = 60.0;
    data2Location.eventClassify = Dynamic_Event;
    PsdLocation::getInstance()->getPsdRoLocation(data2Location);

    // data2Location.headingDelta = 180.0;                                                     //RelativeDir == Dir_ComingFromBehind
    // data2Location.RoPos = {, 0.0};                                                                  //matched root segment, behind HV
    // data2Location.RvSpeed = 60.0;
    // data2Location.eventClassify = Dynamic_Event;
    // PsdLocation::getInstance()->getPsdRoLocation(data2Location);

    // data2Location.headingDelta = 180.0;                                                     //RelativeDir == Dir_ComingFromBehind
    // data2Location.RoPos = {, 0.0};                     //matched parent segment, behind HV
    // data2Location.RvSpeed = 60.0;
    // data2Location.eventClassify = Dynamic_Event;
    // PsdLocation::getInstance()->getPsdRoLocation(data2Location);
    
    //case:matched child segment, actual distance between RO and HV over the  pPsdValidDistance
    data2Location.headingDelta = 90.0;                                                        //RelativeDir == Dir_ComingFromRight
    data2Location.RoPos = {40.355764, 116.812151, 0.0};                     //matched child segment
    data2Location.RvSpeed = 60.0;
    data2Location.eventClassify = Dynamic_Event;
    PsdLocation::getInstance()->getPsdRoLocation(data2Location);//actual distance between RO and HV over the  pPsdValidDistance
}

/*test calcRoDis2Intersection_2*/
TEST_F(PsdLocationTest, calcRoDis2Intersection_2)
{
    struct PsdMapData MapData_1, MapData_2;
    if (PsdMap::getInstance()->getTree() != NULL)
    {
    ...
    tSelfSegment curSegment;
    curSegment.curSegmentId = 4;
    curSegment.curRemainLength = 78;
    curSegment.posIsUnique = true;
    curSegment.posLengthErr = Pos_2mTo5m;
    curSegment.hvHeading = 179.33;
    curSegment.hvCoordinate = {40.356488, 116.815135};
    PsdMessageDecoder::getInstance()->setSelfSegment(curSegment);

    //MapData_2 as HV's child Segment, preSegmentId = 5, prevSegmentId = 4
    MapData_2.preSegmentId = 5;
    MapData_2.prevSegmentId = 4;
    MapData_2.preSegTotalLength = 254;
    MapData_2.psd04RoadClass = CountyRoad;
    MapData_2.endCurvature = 0.0;
    MapData_2.mpp = 1;
    MapData_2.sp = 1;
    MapData_2.startCurvature = 0;
    MapData_2.branchDirection = 0; //right
    MapData_2.branchAngle = +90.06;
    MapData_2.segIsInTree = false;
    MapData_2.accumulateBranchAngle = +90.07;
        
    //save to database: segment 4, 5
    PsdMessageDecoder::getInstance()->getVPsdMap().push_back(&MapData_1);
    PsdMessageDecoder::getInstance()->getVPsdMap().push_back(&MapData_2);
    for (auto it = PsdMessageDecoder::getInstance()->getVPsdMap().begin(); it != PsdMessageDecoder::getInstance()->getVPsdMap().end(); it++)
    {
    ...
    Data2Location data2Location;
    data2Location.headingDelta = 90.0;                                                        //RelativeDir == Dir_ComingFromRight
    data2Location.RoPos = {40.355767, 116.814077, 0.0};                     //matched child segment
    data2Location.RvSpeed = 60.0;
    data2Location.eventClassify = Dynamic_Event;
    PsdLocation::getInstance()->getPsdRoLocation(data2Location);
}
...
TEST_F(PsdLocationTest, RoMatchSegment_curvecase)
{
    struct PsdMapData MapData_1;
    if (PsdMap::getInstance()->getTree() != NULL)
    {
...
TEST_F(PsdLocationTest, accumulateChildsLength)
{
    ...
    EXPECT_EQ(PsdLocation::getInstance()->testaccumulateChildsLength(tempNode), 0);
}
...
TEST_F(PsdLocationTest, calcRoadSegmentClass)
{
    ...
    curSegment.hvCoordinate = {40.358150, 116.815109};  //located at Dongji Road, Miyun District, Beijing, China
    PsdMessageDecoder::getInstance()->setSelfSegment(curSegment);
    EXPECT_EQ(PsdLocation::getInstance()->testcalcRoadSegmentClass(), RoadClass_Unknown);
    ...
    EXPECT_EQ(PsdLocation::getInstance()->testcalcRoadSegmentClass(), RoadClass_highway);
    ...
    EXPECT_EQ(PsdLocation::getInstance()->testcalcRoadSegmentClass(), RoadClass_Non_urban);
    ...
    EXPECT_EQ(PsdLocation::getInstance()->testcalcRoadSegmentClass(), RoadClass_Urban);
    ...
    //case7: HV's segment is not matched mTree, RoadClass_Unknown
...
TEST_F(PsdLocationTest, getPsdAvailable)
{
    ...
    PsdMessageDecoder::getInstance()->setSelfSegment(curSegment);
    EXPECT_EQ(PsdLocation::getInstance()->testgetPsdAvailable(), false);
    ...
    curSegment.posIsUnique = true;
    PsdMessageDecoder::getInstance()->setSelfSegment(curSegment);
    EXPECT_EQ(PsdLocation::getInstance()->testgetPsdAvailable(), true);
...
//test start
TEST_F(PsdLocationTest, startFunction)
{
    PsdLocation::getInstance()->start();
}
...
protected:
    ...
    PsdMessageDecoder *getInstance()
    {
        if (NULL == pInstance)
        {
            pInstance = PsdMessageDecoder::testPsdMessageDecoder();
        }
        return pInstance;
    }
    ...
    void TerDown()
    {
        PsdMessageDecoder::testPsdMessageDecoder(pInstance);
    }
};
...
TEST_F(PsdMessageDecoderTest, segmentManager)
{
    ...
    MapData_1.accumulateBranchAngle = 0.0;
    PsdMessageDecoder::getInstance()->setPsdMapData(MapData_1);
    PsdMessageDecoder::getInstance()->testcalcPsd04Data(MapData_1.psd04RoadClass, MapData_1.endCurvature, MapData_1.startCurvature, MapData_1.branchAngle);
    PsdMessageDecoder::getInstance()->segmentManager(true);
...
{
    printf("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
    std::string filename = "/home/zhao/Documents/Psd_not_use_signalAPI/gtest/src/parsed_data_straightPath.txt";
    ...
    if (file.is_open())
    {
        ...
        while (std::getline(file, line))
        {
            ...
            if (iterPsd04 != keyValueMap.end())
            {
                ...
                {
                    psd04Valid = true;
                    PsdMapData.preSegmentId = static_cast<uint8_t> (std::stoi(iter->second));
                ...
                if ((iter->second != "Fehlerwert") && (iter->second != "keine Segmentinformation vorhanden"))
                {
                    ...
                    {
                        psd04Valid = true;
                        PsdMapData.prevSegmentId = static_cast<uint8_t> (std::stoi(iter->second));
                        printf("[%s] [%d]: prevSegmentId = %u\n", __FUNCTION__, __LINE__, PsdMapData.prevSegmentId);
                    }
                }
                else
                {
                ...
                if (psd04Valid == true)
                {
                    ...
                    PsdMapData.preSegTotalLength = static_cast<uint8_t> (std::stoi(iter->second));
                    printf("[%s] [%d]: preSegTotalLength = %u\n", __FUNCTION__, __LINE__, PsdMapData.preSegTotalLength);
                    ...
                    {
                        PsdMapData.endCurvature = 0.0;
                        PsdMapData.sp = 1; 
                    }
                    else
                    {
                        PsdMapData.sp = 0; 
                        PsdMapData.endCurvature = static_cast<double> (std::stod(iter->second));
                        iter = keyValueMap.find("PSD_Endkruemmung_Vorz");
                        if (iter->second == "Kruemmung_positiv")
                        {
                            PsdMapData.signEndCurvature = 0;
                        }
                        else 
                        if (iter->second == "Kruemmung_negativ")
                        {
                            PsdMapData.signEndCurvature = 1;
                        }
                    ...
                    {
                        PsdMapData.sp = 0; 
                        PsdMapData.startCurvature = static_cast<double> (std::stod(iter->second));
                        iter = keyValueMap.find("PSD_Anfangskruemmung_Vorz");
                        if (iter->second == "positiv")
                        {
                            PsdMapData.signStartCurvature = 0;
                        }
                        else 
                        if (iter->second == "negativ")
                        {
                            PsdMapData.signStartCurvature = 1;
                        }
                    ...
                    {
                        PsdMapData.branchDirection = 0;
                    }
                    else if (iter->second == "links_abzweigende_Strasse")
                    {
                        PsdMapData.branchDirection = 1;
                    }
                    iter = keyValueMap.find("PSD_Abzweigewinkel");
                    PsdMapData.branchAngle = static_cast<double> (std::stod(iter->second));
                    PsdMapData.branchAngle = PsdMapData.branchAngle * (PsdMapData.branchDirection ? -1 : 1);
...
{
    std::string filename = "/home/zhao/Documents/Psd_not_use_signalAPI/gtest/src/parsed_data_turntable(HV on segment 54).txt";
    std::ifstream file(filename);
    std::string line;
    bool psd04Valid = false;
    bool psd05Valid = false;
    ...
    if (file.is_open())
    {
        ...
        printf("######Ro locates the predicted segment of HV######\n");
        Position3D_D RoPoint_1;                                   
        RoPoint_1.lat = 40.343993;
        RoPoint_1.Long =  116.818696;
        RoPoint_1.elevation = 0.0;     
        printf("###### RoPoint_1 behind HV is %d matched ######\n", PsdLocation::getInstance()->testRoMatchSegment(RoPoint_1));

        Position3D_D RoPoint_2;                                   
        RoPoint_2.lat = 40.342985;
        RoPoint_2.Long =  116.819051;
        RoPoint_2.elevation = 0.0;     
        printf("###### RoPoint_2 front of HV not on segment of 50 is %d matched ######\n", PsdLocation::getInstance()->testRoMatchSegment(RoPoint_2));
    
        Position3D_D RoPoint_3;                                   
        RoPoint_3.lat = 40.343166;
        RoPoint_3.Long =  116.819942;
        RoPoint_3.elevation = 0.0;     
        printf("###### RoPoint_3 on segment of 50 is %d matched ######\n", PsdLocation::getInstance()->testRoMatchSegment(RoPoint_3));
    }   
    else    
    {   
        std::cout << "cannot open" << std::endl;
    ...
    filename = "/home/zhao/Documents/Psd_not_use_signalAPI/gtest/src/parsed_data_turntable(HV on segment 58).txt";
    file.open(filename);
    if (file.is_open())
    {
        std::cout << "open successfully" << std::endl;
    ...
    filename = "/home/zhao/Documents/Psd_not_use_signalAPI/gtest/src/parsed_data_turntable(HV on segment 16).txt";
    file.open(filename);
    if (file.is_open())
    {
        std::cout << "open successfully" << std::endl;
