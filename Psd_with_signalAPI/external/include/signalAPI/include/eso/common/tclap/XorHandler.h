...
class XorHandler
{
    ...
    public:
         ...
         */
        XorHandler( ) IPL_NOEXCEPT : _orList(std::vector< std::vector<Arg*> >()) {}

        /**
         * Add a list of Arg*'s that will be xor'd together.
         * \param ors - list of Arg* that will be xor'd.
         */
        void add( std::vector<Arg*>& ors ) IPL_NOEXCEPT;
         ...
         */
        int check( const Arg* a ) IPL_NOEXCEPT;

        /**
         * Returns the XOR specific short usage.
         */
        std::string shortUsage() IPL_NOEXCEPT;

        /**
         * Prints the XOR specific long usage.
         * \param os - Stream to print to.
         */
        void printLongUsage(std::ostream& os) IPL_NOEXCEPT;
         ...
         * \param a - The Arg to be checked.
         */
        bool contains( const Arg* a ) IPL_NOEXCEPT;

        std::vector< std::vector<Arg*> >& getXorList() IPL_NOEXCEPT; 
...
//BEGIN XOR.cpp
//////////////////////////////////////////////////////////////////////
inline void XorHandler::add( std::vector<Arg*>& ors ) IPL_NOEXCEPT
{ 
    _orList.push_back( ors );
}

inline int XorHandler::check( const Arg* a ) IPL_NOEXCEPT 
{
...
inline bool XorHandler::contains( const Arg* a ) IPL_NOEXCEPT
{
    for ( int i = 0; static_cast<unsigned int>(i) < _orList.size(); i++ )
        for ( ArgVectorIterator it = _orList[i].begin(); 
              it != _orList[i].end(); 
...
inline std::vector< std::vector<Arg*> >& XorHandler::getXorList() IPL_NOEXCEPT 
{
    return _orList;
}
