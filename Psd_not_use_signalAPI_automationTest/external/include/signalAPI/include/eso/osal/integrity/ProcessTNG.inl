...
//         m_handle.stdin_fd = pipe.getReadEnd();
    ...
    inline bool ProcessTNG::setPriority(const bool inherit_priority
                                        ,const Priority prio)
    {
        // no ProcessTNG real implementation on integrity so far
        return false;
    }

    inline bool ProcessTNG::setSchedAttrs(const osal::sched::Attr& attr)
    {
        // no ProcessTNG real implementation on integrity so far
        return false;
    }
