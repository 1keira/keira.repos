...
/**
 * According to R17-10 SWS_LOG Section 7.3 the use of the same LogStream
 * object from different threads is not supported without additional
 * synchronization.
 *
 * Operations on LogStream instances will not block.  As a consequence, log
 * messages may be dropped if messages are produced faster than they can be
 * processed by the system.  The details about how to handle this scenario are
 * not defined by this API specification.
 */
 ...
 * nevertheless part of the System-API to ensure backwards compatibility with
...
namespace log
{
    using eso_ara_log_backport::LogStream;
} // namespace log
} // namespace ara
#endif
